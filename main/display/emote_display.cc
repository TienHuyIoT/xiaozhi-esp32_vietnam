...
static void SetupUI(const gfx_handle_t engine_handle, EmoteDisplay* const display)
{
    ...
    gfx_obj_align(g_obj_img_status, GFX_ALIGN_RIGHT_MID, -10, 30);
...
{
    if (!display) {
        ESP_LOGE(TAG, "EmoteEngine: display is nullptr");
        return;
    }

    InitializeGraphics(panel, &engine_handle_, width, height);
    if (!engine_handle_) {
        ESP_LOGE(TAG, "EmoteEngine: Failed to initialize graphics");
        return;
    }
    ...
    esp_lcd_panel_io_register_event_callbacks(panel_io, &cbs, nullptr);
}
