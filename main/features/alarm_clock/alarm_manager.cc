    ...
    for (uint8_t i = 0; i < count && i < 10; i++) {
        ...
        if (err == ESP_OK && required_size > 0) {
            ...
            if (parsed == 2) {
                ...
                if (msg_start) {
                    msg_start++;
                    char* sound_start = strchr(msg_start, '|');
                    if (sound_start) {
                        *sound_start = '\0';
                        alarm.message = msg_start;
                        char* flag_start = strchr(sound_start + 1, '|');
                        if (flag_start) {
                            *flag_start = '\0';
                            alarm.sound_id = sound_start + 1;
                            alarm.enabled = (flag_start[1] == '1');
                            alarm.repeated = (flag_start[3] == '1');
                            alarms_.push_back(alarm);
                            ESP_LOGI(TAG, "Loaded alarm: %02d:%02d - %s [sound=%s] (enabled: %d, repeated: %d)",
                                    alarm.hour, alarm.minute, alarm.message.c_str(),
                                    alarm.sound_id.c_str(),
...
void AlarmManager::saveToNVS() {
    ...
    for (size_t i = 0; i < alarms_.size(); i++) {
        ...
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "%02d:%02d|%s|%s|%d|%d",
                 alarms_[i].hour, alarms_[i].minute,
                 alarms_[i].message.c_str(),
                 alarms_[i].sound_id.c_str(),
...
void AlarmManager::parseResponse(const std::string& text) {
    ...
    new_alarm.sound_id = "alarm"; // default
    ...
    // Parse sound_id từ keyword
    if (text.find("di hoc") != std::string::npos ||
        text.find("đi học") != std::string::npos ||
        text.find("di lam") != std::string::npos ||
        text.find("đi làm") != std::string::npos ||
        text.find("school") != std::string::npos ||
        text.find("work") != std::string::npos) {
        new_alarm.sound_id = "school";
    } else if (text.find("thuc day") != std::string::npos ||
               text.find("thức dậy") != std::string::npos ||
               text.find("day") != std::string::npos ||
               text.find("dậy") != std::string::npos ||
               text.find("wake") != std::string::npos) {
        new_alarm.sound_id = "wakeup";
    } else if (text.find("uong thuoc") != std::string::npos ||
               text.find("uống thuốc") != std::string::npos ||
               text.find("thuoc") != std::string::npos ||
               text.find("thuốc") != std::string::npos ||
               text.find("medicine") != std::string::npos) {
        new_alarm.sound_id = "medicine";
    }
    ...
    ESP_LOGI(TAG, "Added alarm: %02d:%02d - %s [sound=%s] (repeated: %d)",
             hour, minute, new_alarm.message.c_str(),
             new_alarm.sound_id.c_str(), new_alarm.repeated);
}
...
void AlarmManager::addAlarm(const Alarm& alarm) {
    ...
    // Parse sound_id từ keyword trong message (neu chua duoc dat)
    if (new_alarm.sound_id == "alarm" && !new_alarm.message.empty()) {
        const std::string& text = new_alarm.message;
    ...
    alarms_.push_back(new_alarm);
    saveToNVS();
    ESP_LOGI(TAG, "Added alarm via MCP: %02d:%02d [sound=%s]", new_alarm.hour, new_alarm.minute,
             new_alarm.sound_id.c_str());
}

bool AlarmManager::setAlarmSound(uint8_t hour, uint8_t minute, const std::string& sound_id) {
    for (auto& alarm : alarms_) {
        if (alarm.hour == hour && alarm.minute == minute) {
            alarm.sound_id = sound_id;
            saveToNVS();
            ESP_LOGI(TAG, "Updated alarm %02d:%02d sound_id to '%s'",
                     hour, minute, sound_id.c_str());
            return true;
        }
    }
    return false;
...
void AlarmManager::triggerAlarm(const Alarm& alarm) {
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "ALARM TRIGGERED: %02d:%02d [sound=%s]",
             alarm.hour, alarm.minute, alarm.sound_id.c_str());
    ESP_LOGI(TAG, "Message: %s", alarm.message.c_str());
    ESP_LOGI(TAG, "========================================");
    ...
    std::string sound_id = alarm.sound_id;

    app.Schedule([&app, sound_id]() {
        // Lấy OGG từ sound_id (fallback về alarm nếu không tìm thấy)
        std::string_view beep_sound = GetAlarmSoundOgg(sound_id);

        // Phát 5 lần
        for (int i = 0; i < 5; i++) {
            ESP_LOGI(TAG, "Playing alarm sound=%s [%d/5]", sound_id.c_str(), i + 1);
...
std::string AlarmManager::getAllAlarmsInfo() {
    ...
    for (const auto& [diff, alarm] : sorted_alarms) {
        ...
        snprintf(buffer, sizeof(buffer), "%d. %02d:%02d - %s [%s]%s (còn %dh%02dp)\n",
                 count++,
                 alarm->hour,
                 alarm->minute,
                 alarm->message.c_str(),
